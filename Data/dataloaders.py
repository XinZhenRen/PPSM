...
from Data.dataset import SegDataset,PromptDataset,VideoDataset


def split_ids(len_ids):
    print("len_ids:", len_ids)
...
def get_dataloaders(input_paths:list, target_paths:list, batch_size,
                    prompt:str=None,resize=352):#->train_dataloader, test_dataloader, val_dataloader,prompt_dataloader
    ...
    transform_target = transforms.Compose(
        [transforms.ToTensor(), transforms.Resize((resize, resize)), transforms.Grayscale()]
    ...
    if prompt is None:
        ...
        transform_prompt = transforms.Compose(
            [transforms.ToTensor(),
             transforms.Resize((resize, resize)), transforms.Grayscale()]
            # [transforms.ToTensor(), transforms.Resize((352, 352))]
        )



        train_dataset = PromptDataset(input_paths=input_paths,
                                             target_paths=target_paths,
                                             prompt_path=prompt,
                                             transform_input=transform_input4train,
                                             transform_target=transform_target,
        ...
        test_dataset = PromptDataset(input_paths=input_paths,
                                             target_paths=target_paths,
                                             prompt_path=prompt,
                                             transform_input=transform_input4test,
                                             transform_target=transform_target,
        ...
        val_dataset = PromptDataset(input_paths=input_paths,
                                            target_paths=target_paths,
                                            prompt_path=prompt,
                                            transform_input=transform_input4test,
                                            transform_target=transform_target,
