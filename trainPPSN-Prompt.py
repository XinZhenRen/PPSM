...
#python trainPPSN-Prompt.py --model=PPSN --dataset=Kvasir --data-root=./kvasir-seg/
#python trainPPSN-Prompt.py --model=PPSN --dataset=CVC --data-root=./CVC-ClinicDB/
#python trainPPSN-Prompt.py --model=PPSN --dataset=SUN --data-root=./SUN/
#python trainPPSN-Prompt.py --model=PPSN --dataset=polyGen --data-root=./polypGen2021_MultiCenterData_v3/
...
def test(model, device, test_loader, epoch, perf_measure):
    ...
    for batch_idx, (data, prompt,target) in enumerate(test_loader):
        data,prompt, target = data.to(device),prompt.to(device), target.to(device)
        output = model(data,prompt)
        #writer.add_scalar('MEA',perf_measure(output, target).item(),global_step=epoch)
...
def train_epoch(model, device, train_loader, optimizer, epoch, Dice_loss, BCE_loss,prompt_loader=None):
    try:
        t = time.time()
        model.train()
        loss_accumulator = []
        ...
        for batch_idx, (data,prompt, target) in enumerate(train_loader):
            #without try#########################################################
            data,prompt, target = data.to(device),prompt.to(device) ,target.to(device)
            optimizer.zero_grad()
            output = model(data,prompt)
...
def train(args):
    ...
    train_dataloader, _, val_dataloader = dataloaders.get_dataloaders(
        input_paths, target_paths,prompt=prompt_path, batch_size=args.batch_size)
        ...
        for epoch in bar(range(Nepoch, args.epochs + 1)):
            ...
            if prev_best_test == None or test_measure_mean > prev_best_test:
                ...
                torch.save(
                    ...
                    "Trained_models/{}_".format(args.model) + args.dataset +"_epoch"+str(epoch)+"_mean"+str(test_measure_mean)+"_device"+str(device)+"_p"+str(args.p)+ ".pt",
                )
...
def get_args():
    ...
    parser.add_argument("--data-root", type=str, default="./Kvasir-seg/",required=True, dest="root")
